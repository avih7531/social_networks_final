...
def plot_actor_network(
    ...
    color_map = {
        com: community_colors[i] for i, com in enumerate(sorted(communities))
    }
...
def plot_accuracy_across_samples(
    cross_sample_data: Dict, output_dir: str
) -> None:
    """
...
def plot_pagerank_distribution_comparison(
    pagerank_data: Dict, output_dir: str
) -> None:
    """
    ...
    bins = np.logspace(
        np.log10(min(all_pr_values)), np.log10(max(all_pr_values)), 40
    )
